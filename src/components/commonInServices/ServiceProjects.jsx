            ...
            <div className='flex flex-col gap-12 '>
                {projects.map((project, index) => (
                        <div  key={project.title} className={`flex  w-full flex-col-reverse gap-8 sm:gap-0 items-center sm:items-start sm:flex-row justify-between p-3 sm:p-5  trans rounded-md  border-[1px]`}>
                            ...
                            <div className='h-64 w-64 trans overflow-hidden rounded-md border-[1px] bg-main-color '>
                                <Img onMouseEnter={enterImg} onMouseLeave={leaveImg} className=" transition-all duration-[2500ms] w-full" loader={<div className='w-full h-full bg-zinc-50 border-[1px] flex items-center justify-center rounded-md'><LoadingSpinner /></div>} src={project.pic} alt={project.title}  />
                            </div>
                        </div>
                ))}
